...
class Figura:
    ...
    def pedirNum(self, mensaje):
        while True:
            valor = input(mensaje)
            if valor.replace('.','',1).isdigit(): #asi puedo incluir decimales porque el .isdigit no toma ,
                return float(valor)               #por eso se reemplazan para que el numero quede junto
...
opciones={
    ...
    '4':('Cuadrado', Cuadrado),
    '5':('Salir', None)
} #me hago un diccionario que mapea las opciones a tuplas
...
while True:
    ...
    if op == '5':                #ejem: ('Rectangulo', Rectangulo = 2)
        print('Saliendo...')
        break
    figura = clase()             #se usa aqui para saber que metodo usar
